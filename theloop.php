	...
	?>
		...
		<div id="post-<?php the_ID(); ?>" <?php post_class("post-caption"); ?>>
			...
			<?php } else { ?>
				<h1><?php the_title(); ?></h1>
			<?php } ?>
			<p <?php if(is_page()) echo 'style="display:none"' ?>><?php if (!is_page()) {?><span class='post-date'><?php echo get_the_date(); ?></span> <img alt="" src="<?php echo get_template_directory_uri(); ?>/images/smt/category.png">Category:&nbsp;<?php the_category(',&nbsp;'); }?>
